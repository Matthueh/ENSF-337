...
void String_append(String *dest, const String* source){
	int new_length =  (*dest).length + (*source).length;
	char * copy = malloc(new_length+1);
	if(copy == NULL)
	{
		printf("The memory that has been allocated has failed");
		exit(1);
	}
	...
	int j;
	int k;
	for(i = 0, j = 0, k = 0; k < new_length; k++)
	{
		if( (*dest).dynamic_storage[i] != '\0')
		{	
			copy[k] = (*dest).dynamic_storage[i];
			i++;
		}
		
		else if( (*source).dynamic_storage[j] != '\0')
		{
			copy[k] = (*source).dynamic_storage[j];
			j++;
		}
	}
	copy[k] = '\0';
...
{	
	...
	if(copy == NULL)
	{
		printf("The bytes for copy have not yet been allocated");
		exit(1);
	}
	
	for(i = 0; i < new_length; i++)
	{
		copy[i] = (*dest).dynamic_storage[i];
	}
