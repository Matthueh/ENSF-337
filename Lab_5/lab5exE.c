...
int main(void)
{
    Point alpha = { "A1", 2.3, 4.5, 56} ;
    Point beta = { "B1", 25.9, 30.0, 97} ;
...
void swap(Point* p1, Point *p2)
{
	...
	while((*p1).label[i]  != '\0' && (*p2).label[i] != '\0')
	{
		(storage).label[i] = (*p1).label[i];
		(*p1).label[i] = (*p2).label[i];
		(*p2).label[i] = (storage).label[i];
		i++;
	}
